...
./client -l /tmp/mbox_socket -c 0:0:50000
if [[ $((256-$?)) != $OP_PUT_SIZE ]]; then
    exit 1
fi

# inserisco 2 oggetti di size massima
./client -l /tmp/mbox_socket -c 0:0:32768 -c 1:0:32768
if [[ $? != 0 ]]; then
    exit 1
fi

./client -l /tmp/mbox_socket -c 2:0:1
if [[ $((256-$?)) != $OP_PUT_REPOSIZE ]]; then
    exit 1
fi

# rimuovo gli oggetti inseriti 
./client -l /tmp/mbox_socket -c 0:3:0 -c 1:3:0
if [[ $? != 0 ]]; then
    exit 1
fi
...
# PUT
for ((i=0;i<1024;++i)); do 
    ./client -l /tmp/mbox_socket -c $i:0:63
    if [[ $? != 0 ]]; then
	exit 1
    fi
done

./client -l /tmp/mbox_socket -c 1024:0:1
...
# GET
for ((i=0;i<1024;++i)); do 
    ./client -l /tmp/mbox_socket -c $i:2:0
    if [[ $? != 0 ]]; then
	exit 1
    fi
done
...
# UPDATE
for ((i=0;i<1024;++i)); do 
    ./client -l /tmp/mbox_socket -c $i:1:63
    if [[ $? != 0 ]]; then
	exit 1
    fi
done

./client -l /tmp/mbox_socket -c 0:1:62
...
# REMOVE
for ((i=0;i<1024;++i)); do 
    ./client -l /tmp/mbox_socket -c $i:3:0
    if [[ $? != 0 ]]; then
	exit 1
    fi
done
...
sleep 1
./client -l /tmp/mbox_socket -c 12:0:1024
e=$?
